const zhHans = {
  ...
  bots: {
    ...
    kuku: {
      title: 'Discord KUKU',
      description: '为 Discord 文本频道启用并配置 KUKU 主动聊天设置。',
      loadBootstrapError: '加载 KUKU 配置数据失败：',
      loadSettings: '加载设置',
      loadingSettings: '加载中...',
      loadSuccess: 'KUKU 设置已加载',
      loadError: '加载 KUKU 设置失败：',
      saveSuccess: 'KUKU 设置已保存',
      saveError: '保存 KUKU 设置失败：',
      saveSettings: '保存 KUKU 设置',
      groupId: 'Discord 频道 ID',
      groupIdPlaceholder: '粘贴 Discord 文本频道 ID',
      groupIdRequired: 'Discord 频道 ID 不能为空',
      channelHint:
        '当前 MVP 的 KUKU 仅支持 Discord。请使用文本频道 ID 作为群组范围，然后加载已有设置或保存新设置。',
      persona: '人格',
      enabled: '启用',
      enabledHint: '允许 KUKU 在该频道主动发送消息。',
      silenceMinutes: '静默分钟数',
      cooldownMinutes: '冷却分钟数',
      quietHours: '免打扰时段',
      quietHoursHint: '可选。不填写则禁用该频道的免打扰时段。',
      quietStart: '开始',
      quietEnd: '结束',
      timezone: '时区',
      loadedState: '已加载该频道保存的 KUKU 设置。',
      missingState: '该频道尚未保存 KUKU 设置。保存后将创建新配置。',
      idleState: '输入 Discord 频道 ID 以加载现有设置或创建新配置。',
      notConfiguredYet: '该频道暂未找到已保存的 KUKU 设置。',
    },
    sessionMonitor: {
      title: '会话监控',
      sessions: '会话列表',
