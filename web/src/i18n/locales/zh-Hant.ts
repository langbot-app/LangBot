const zhHant = {
  ...
  systemSettings: {
    title: '系統設定',
    description: '設定日誌、監控記錄與執行期會話的自動清理策略。',
    loadFailed: '載入系統設定失敗',
    saveFailed: '儲存系統設定失敗',
    saved: '系統設定已儲存',
    autoCleanupEnabled: '啟用自動清理',
    autoCleanupEnabledDescription: '依照設定週期自動清理舊日誌、監控記錄與長時間閒置的執行期會話。',
    intervalHours: '執行週期（小時）',
    intervalHoursDescription: '背景任務會依照這個週期檢查並執行自動清理。',
    logRetentionDays: '日誌保留天數',
    monitoringRetentionDays: '監控記錄保留天數',
    monitoringRetentionDaysDescription: '會清理監控頁面中的訊息、會話、模型呼叫與錯誤記錄。',
    runtimeSessionIdleHours: '執行期會話閒置時長（小時）',
    runtimeSessionIdleHoursDescription: '超過此時長未活動的記憶體會話會被釋放，可能導致多輪上下文遺失。',
    zeroDisables: '填 0 表示停用這一項清理。',
    notice: '自動清理會影響歷史排障資料與長會話上下文，建議先從較保守的保留時間開始。',
    validation: {
      intervalHours: '執行週期必須是大於 0 的整數',
      nonNegativeInteger: '所有清理設定都必須是大於等於 0 的整數',
    },
  },
  account: {
    settings: '帳戶設定',
