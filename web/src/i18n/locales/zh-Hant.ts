const zhHant = {
  ...
  bots: {
    ...
    kuku: {
      title: 'Discord KUKU',
      description: '為 Discord 文字頻道啟用並設定 KUKU 主動聊天功能。',
      loadBootstrapError: '載入 KUKU 設定資料失敗：',
      loadSettings: '載入設定',
      loadingSettings: '載入中...',
      loadSuccess: 'KUKU 設定已載入',
      loadError: '載入 KUKU 設定失敗：',
      saveSuccess: 'KUKU 設定已儲存',
      saveError: '儲存 KUKU 設定失敗：',
      saveSettings: '儲存 KUKU 設定',
      groupId: 'Discord 頻道 ID',
      groupIdPlaceholder: '貼上 Discord 文字頻道 ID',
      groupIdRequired: 'Discord 頻道 ID 不可為空',
      channelHint:
        '目前 MVP 的 KUKU 僅支援 Discord。請使用文字頻道 ID 作為群組範圍，然後載入既有設定或儲存新設定。',
      persona: '人格',
      enabled: '啟用',
      enabledHint: '允許 KUKU 在此頻道主動發送訊息。',
      silenceMinutes: '靜默分鐘數',
      cooldownMinutes: '冷卻分鐘數',
      quietHours: '安靜時段',
      quietHoursHint: '選填。留空即可停用此頻道的安靜時段。',
      quietStart: '開始',
      quietEnd: '結束',
      timezone: '時區',
      loadedState: '已載入此頻道已儲存的 KUKU 設定。',
      missingState: '此頻道尚未有已儲存的 KUKU 設定。儲存後將建立設定。',
      idleState: '輸入 Discord 頻道 ID 以載入現有設定或建立新設定。',
      notConfiguredYet: '此頻道尚未找到已儲存的 KUKU 設定。',
    },
    sessionMonitor: {
      title: '會話監控',
      sessions: '會話列表',
