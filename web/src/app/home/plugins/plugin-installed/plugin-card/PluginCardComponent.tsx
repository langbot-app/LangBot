      ...
      <div className="w-full h-full flex flex-row items-start justify-start gap-[1.2rem]">
        ...
        <div className="flex flex-col items-center justify-between h-full">
          ...
          <div className="flex items-center justify-center">
            ...
            <svg
                className={`w-[1.4rem] h-[1.4rem] cursor-pointer ${cardVO.repository ? 'text-black' : 'text-gray-400'}`}
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
                onClick={(e) => {
                  e.stopPropagation(); // 阻止事件冒泡
                  if (cardVO.repository && cardVO.repository.trim() && cardVO.repository.startsWith('http')) { 
                    window.open(cardVO.repository, '_blank');
                  } else {
                    toast.error(t("plugins: The githubUrl wasn't save"));
                  }
                }}
              >
