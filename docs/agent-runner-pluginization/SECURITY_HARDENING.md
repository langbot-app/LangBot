...
| Skill projection policy | Partial | Host resource builder 会按 runner capability 和 resource policy 暴露 skill 名称；当前 code-agent runner 不再接受用户手写 `skills-json`，避免 runner binding 任意投影 skill；skill tool 路径和可见性已有部分单测。 | 缺 code-agent harness 的发布级只读 skill 投影、skill 来源验证、版本 / hash 记录、projection cleanup 和审计；如果后续恢复 native skill 投影，需要纳入 Host 授权资源而不是自由 JSON。 |
