    ...
    async def _call_webhook(self, query: core_entities.Query) -> typing.AsyncGenerator[llm_entities.Message, None]:
        ...
        try:
            ...
            if self.auth_type == 'basic':
                ...
                self.ap.logger.debug(f'使用Basic认证: {self.basic_username}')
            elif self.auth_type == 'jwt':
                # 使用JWT认证
                import jwt
                import time
                ...
                self.ap.logger.debug('使用JWT认证')
            elif self.auth_type == 'header':
                # 使用自定义请求头认证
                headers[self.header_name] = self.header_value
                self.ap.logger.debug(f'使用Header认证: {self.header_name}')
            else:
                self.ap.logger.debug('不使用认证')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        self.ap.logger.error(f'N8n webhook调用失败: {response.status}, {error_text}')
                        raise Exception(f'N8n webhook调用失败: {response.status}, {error_text}')
                    
                    # 解析响应
                    response_data = await response.json()
                    self.ap.logger.debug(f'N8n webhook响应: {response_data}')
            ...
            self.ap.logger.error(f'N8n webhook调用异常: {str(e)}')
            # 返回错误消息
            yield llm_entities.Message(
                role='assistant',
                content=f'调用N8n工作流时发生错误: {str(e)}',
            )
