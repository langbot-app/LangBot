...
class LangflowAPIRunner(runner.RequestRunner):
    ...
    async def _build_request_payload(self, query: core_entities.Query) -> dict:
        ...
        input_type = self.pipeline_config.get('langflow', {}).get('input_type', 'chat')
        output_type = self.pipeline_config.get('langflow', {}).get('output_type', 'chat')
        ...
        tweaks = self.pipeline_config.get('langflow', {}).get('tweaks')
        if tweaks:
    ...
    async def run(self, query: core_entities.Query) -> typing.AsyncGenerator[llm_entities.Message | llm_entities.MessageChunk, None]:
        ...
        # 从配置中获取API参数
        base_url = self.pipeline_config['langflow']['base-url']
        api_key = self.pipeline_config['langflow']['api-key']
        flow_id = self.pipeline_config['langflow']['flow-id']
            ...
            try:
                if is_stream:
                    ...
                    if is_stream:
                        ...
                        reply_message = llm_entities.Message(
                            role="assistant",
                            content=message_text
                        )
                        yield reply_message
